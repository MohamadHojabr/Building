...
namespace Building.Common.Helpers
{
    ...
    public static class PersianDate
    {
        ...
        public static DateTime ToGregorianDate(string persianDateTime)
        {
            ...
            var lastPart = persianDateSplit[2];
            var day = lastPart;
            var hour = 0;
            var min = 0;
            if (lastPart.Length > 2)
            {
                var lastPartSplit = lastPart.Split(new[] { " " }, StringSplitOptions.RemoveEmptyEntries);
                day = lastPartSplit[0];
                hour = Convert.ToInt32(lastPartSplit[1].Split(':')[0]);
                min = Convert.ToInt32(lastPartSplit[1].Split(':')[1]);
            }

            var gregorianDate = pcal.ToDateTime(Convert.ToInt32(persianDateSplit[0]), Convert.ToInt32(persianDateSplit[1]), Convert.ToInt32(day), hour, min, 0, 0);
